    ...
    <div className="container" style={{ padding: '24px', maxWidth: '600px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <button 
                ...
                style={{ background: 'rgba(85, 70, 255, 0.1)', border: '1px solid #5546ff', color: '#5546ff', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer' }}
            >?</button>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div className="card shadow" onClick={() => setShowRawUnits(!showRawUnits)} style={{ textAlign: 'center', borderTop: '4px solid #fc6432', padding: '15px', cursor: 'pointer' }}>
            ...
            <h2 style={{ margin: '5px 0', color: '#fc6432' }}>
              ...
              <span style={{fontSize: '0.6rem', opacity: 0.5}}>{showRawUnits ? ' uSTX' : ' STX'}</span>
            </h2>
          </div>
          <div className="card shadow" onClick={() => setShowRawUnits(!showRawUnits)} style={{ textAlign: 'center', borderTop: '4px solid #f7931a', padding: '15px', cursor: 'pointer' }}>
            ...
            <h2 style={{ margin: '5px 0', color: '#f7931a' }}>
              ...
              <span style={{fontSize: '0.6rem', opacity: 0.5}}>{showRawUnits ? ' Sats' : ' BTC'}</span>
            </h2>
          </div>
        </div>
      )}
        ...
        {!userData ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{textAlign: 'center', marginBottom: '10px'}}>
               ...
               <button onClick={handleDisconnect} style={{background: 'none', border: 'none', color: '#ff4b4b', fontSize: '0.65rem', cursor: 'pointer', textDecoration: 'underline'}}>Disconnect Wallet</button>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '5px 0' }}>
               <input type="checkbox" id="terms" checked={agreedToTerms} onChange={(e) => setAgreedToTerms(e.target.checked)} style={{ cursor: 'pointer' }}/>
               <label htmlFor="terms" style={{ fontSize: '0.75rem', opacity: 0.7 }}>
                  I agree to the <span onClick={() => setShowTerms(true)} style={{ color: '#5546ff', cursor: 'pointer', textDecoration: 'underline' }}>Terms</span>
               </label>
            </div>

            <button className="primary" onClick={createInvoice} disabled={loading || !amount || !agreedToTerms}>
              {loading ? 'Confirming...' : 'Generate Invoice Link'}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="card shadow" style={{ padding: '20px', marginBottom: '24px', borderLeft: '4px solid #fc6432' }}>
        ...
        <div style={{ maxHeight: '250px', overflowY: 'auto' }}>
          {filteredOpen.length === 0 ? <p style={{ opacity: 0.5, fontSize: '0.8rem' }}>No open invoices found.</p> : (
            filteredOpen.map((tx: any) => (
              <div key={tx.tx_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <button className="secondary" onClick={() => navigator.clipboard.writeText(`${window.location.origin}/pay/${tx.tx_id}`)} style={{ padding: '4px 10px', fontSize: '0.7rem' }}>Copy Link</button>
              </div>
            ))
          )}
        </div>
      ...
      <div className="card shadow" style={{ padding: '20px', borderLeft: '4px solid #28a745' }}>
        ...
        <div style={{ maxHeight: '250px', overflowY: 'auto' }}>
          {filteredPaid.length === 0 ? <p style={{ opacity: 0.5, fontSize: '0.8rem' }}>No payments found.</p> : (
            filteredPaid.map((tx: any) => (
              <div key={tx.tx_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <a href={`https://explorer.hiro.so/txid/${tx.tx_id}?chain=mainnet`} target="_blank" rel="noreferrer" style={{ fontSize: '0.7rem', color: '#5546ff', textDecoration: 'none' }}>View Explorer ↗</a>
              </div>
            ))
          )}
        </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="card shadow" style={{ maxWidth: '400px', width: '100%', padding: '24px', background: '#121212', border: '1px solid #5546ff', position: 'relative' }}>
             ...
             <div style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.7)', lineHeight: '1.6' }}>
                <p>Create an invoice to generate a unique link. Once paid, funds settle on the Stacks blockchain.</p>
      ...
      {showTerms && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.95)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="card shadow" style={{ maxWidth: '450px', maxHeight: '80vh', overflowY: 'auto', padding: '30px', background: '#121212' }}>
            <h3 style={{ color: '#fc6432' }}>Terms of Service</h3>
            <div style={{ fontSize: '0.8rem', lineHeight: '1.6', opacity: 0.8 }}>
              <p><strong>1. Non-Custodial:</strong> You are responsible for your own private keys and sBTC assets.</p>
              <p><strong>2. Irreversibility:</strong> Blockchain payments cannot be reversed once confirmed.</p>
              <p><strong>3. Liability:</strong> We provide this tool "as-is" and are not responsible for lost funds due to user error.</p>
            </div>
      ...
      {showPrivacy && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.95)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="card shadow" style={{ maxWidth: '450px', padding: '30px', background: '#121212' }}>
            <h3 style={{ color: '#28a745' }}>Privacy Policy</h3>
            <p style={{ fontSize: '0.8rem', opacity: 0.8 }}>We do not store your private data. All invoice and payment data is pulled directly from the public Stacks blockchain ledger.</p>
