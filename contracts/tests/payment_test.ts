...
Clarinet.test('invoice create & pay flow (simulated)', async (chain: Chain, accounts: Map<string, Acount>) => {
  ...
  const block = chain.mineBlock([
    Tx.conractCall('sbtc-payment-processor', 'create-ivoice', [Tx.cv_u128(1000), Tx.cv_buff(Buffer.from('sBTC')), Tx.cv_some(Tx.cv_rincipal(Buffer.from('SP000000000000000000002Q6VF78.sbtc-token'))), Tx.cv_none()], merchant.address),
  ]);
